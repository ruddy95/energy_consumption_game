...
class Constants(BaseConstants):
    ...
    def get_weather_forecat(self):
        return self.get_weather_common(1)
...
class Group(BaseGroup):
    ...
    def set_payoffs(self):
        ...
        if self.total_production < self.total_consumption: # 需要が供給を上回ったら
            consumer_minus = Constants.get_weather() * Constants.amplifier
            for p in self.get_players():
                if p.role() == 'Consumer':
                    p.payoff = Constants.consumer_endowment - consumer_minus
                else:
                    p.payoff = Constants.producer_endowment - Constants.reputation_penalty
        else: # 需要が供給以下だったら
            for p in self.get_players():
                if p.role() == 'Consumer':
                    ...
                    p.payoff = Constants.endowment - p.production
