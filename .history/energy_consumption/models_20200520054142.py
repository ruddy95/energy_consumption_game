    ...
    def set_payoffs(self):
        ...
        if self.total_production < self.total_consumption: # 需要が供給を上回ったら
            ...
            for p in self.get_players():
                if p.role() == 'Consumer':
                    p.payoff = Constants.endowment - consumer_minus
                else:
                    p.payoff = Constants.endowment - Constants.reputation_penalty
        else: # 需要が供給以下だったら
            for p in self.get_players():
                consumer_minus = lambda regression: max(0, Constants.get_weather() - p.consumption) * Constants.amplifier + regression * Constants.regression_amplifier
