...
class MyPage(Page):
    ...
    def vars_for_template(self):
        ...
        forecast_list = self.session.vars['weather_forecast'][p2][q2:]
        time_span = 24 // 3 # calc
        start_time = (3-len(forecast_list)) * 

        return dict(
            date=p1+1,
            start_time=q1*time_span,
            end_time=,
            penalty_for_producer=Constants.penalty * 3,
            weather_forecast=self.session.vars['weather_forecast'][p1][q1] * 3,
            forecast_list=map(lambda x: dict(start_time=,end_time=,value=x), )
        )
