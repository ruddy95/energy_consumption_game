...
class Subsession(BaseSubsession):
    def creating_session(self):
        ...
        for p in self.get_players():
            if p.role() == 'Consumer':
                Constants.endowment = c(1000)
            else:
                Constants.endowment = c(3000)
        ...
        if self.is_blackout:
            for p in self.get_players():
                if p.role() == 'Consumer':
                    ...
                    p.payoff = Constants.endowment - p.usage
                else:
            ...
            for p in self.get_players():
                if p.role() == 'Consumer':
                    regression_cost = Constants.regression_const if self.subsession.round_number > 1 and p.in_previous_rounds()[-1].consumption > p.consumption else 0
